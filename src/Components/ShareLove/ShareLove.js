...
const ShareLove = () => {
  ...
  const handleLove = () => {
    if (user.email) {
      ...
      axios
        .put(
          `https://jhon-portfolio-server.vercel.app/blog/love?id=${blogDetails._id}`,
          user
        )
        .then((res) => {});
    }
  ...
  return (
    <div className="flex justify-between">
      <div
        onClick={handleShare}
        className="text-gray-300 flex hover:text-white transition-colors cursor-pointer mr-4"
      >
        <ShareIcon></ShareIcon>
      ...
      <div className="text-gray-300 hover:text-white transition-colors cursor-pointer flex mr-4">
        {!user.email ? (
          <a
            href="#commentlogin"
            className="text-white hover:no-underline flex"
          >
          ...
          <>
            {blogDetails.love?.filter((single) => single.email === user.email)
              .length ? (
              <div className="flex">
                <FavoriteIcon sx={{ color: "yellow " }}></FavoriteIcon>
                <span className="ml-2 font-bold">
                  {blogDetails?.love?.length}
                </span>
              </div>
            ) : (
              <div className="flex" onClick={handleLove}>
                <FavoriteBorderIcon></FavoriteBorderIcon>
                <span className="ml-2 font-bold">
                  {blogDetails?.love?.length}
                </span>
      ...
      <div className="text-gray-300 flex hover:text-white transition-colors cursor-pointer ">
        <a
          href="#commentlogin"
          className=" text-gray-300 transition-colors  hover:text-white flex hover:no-underline	"
        >
