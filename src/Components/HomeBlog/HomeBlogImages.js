...
const HomeBlogImages = () => {
  ...
  // const { data, error } = useSWR('https://jhon-portfolio-server.vercel.app/blog?short=true', fetcher)
    ...
    if (scrollValue >= 7) {
      if (homeBlog?.length) {
        if (window.innerWidth <= 600) {
          ...
          wrapper2.current.style.transform = `translate3d(0px, ${
            (scrollValue - 7) * 200
          }px, 0px)`;
        } else {
          wrapper.current.style.transform = `translate3d(0px, -${
  ...
  return (
    <motion.div
      exit={{ opacity: 0 }}
      className="h-full font-family-Helvetica text-justify"
    >
      <Grid container className="h-full" spacing={4}>
        <Grid item className="h-full" xs={6}>
          <div ref={wrapper} className="blog-wrap  cursor-pointer">
            {homeBlog?.slice(0, 5).map((single, i) => (
              <BlogImageSingle key={i} {...single}></BlogImageSingle>
            ))}
          </div>
        </Grid>
        <Grid item xs={6} className=" h-full relative">
          <div
            ref={wrapper2}
            className="blog-wrap  cursor-pointer absolute second"
          >
            {homeBlog?.slice(0, 5).map((single, i) => (
              <BlogImageSingle key={i} {...single}></BlogImageSingle>
            ))}
