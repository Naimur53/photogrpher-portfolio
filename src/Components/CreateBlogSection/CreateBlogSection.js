...
const CreateBlogSection = ({
  ...
  useEffect(() => {}, [watch(`file${singleSec.num}`)]);

  //handle complete
  useEffect(() => {
    if (
      watch(`title${singleSec.num}`) &&
      watch(`description${singleSec.num}`)
    ) {
    ...
    watch(`title${singleSec.num}`),
    watch(`description${singleSec.num}`),
    watch(`photosFile${singleSec.num}`),
  ...
  const handleVideoUpload = (e) => {
    ...
    if (file?.length) {
      ...
      // sending to api
      axios
        .post("https://jhon-portfolio-server.vercel.app/video", formData, {
          headers: {
  ...
  return (
    <>
      <Grid item xs={12} md={12}>
        <input
          className="w-full p-3 rounded-lg text-white  bg-gray-900 placeholder:text-slate-400"
          placeholder="Enter Title"
          {...register(`title${singleSec.num}`, { required: true })}
        />
      </Grid>
      <Grid item xs={12} md={6}>
        <textarea
          ...
          {...register(`description${singleSec.num}`, { required: true })}
          rows="6"
        ></textarea>
      </Grid>
      <Grid item xs={12} md={6}>
        {/* hidden input tags */}
        <input
          onChange={handlePhotosFile}
          className="hidden"
          id={"photosFile" + singleSec.num}
          type="file"
          accept="image/*"
          multiple={true}
        />
        <input
          ...
          className="hidden"
          type="file"
          id={"file" + singleSec.num}
          accept="video/*"
        />
        {/* design start */}
        <Box className="h-40 relative bg-gray-900 bg-center flex justify-center items-center">
          {photosLoading || videoLoading ? (
            <CircularProgress
              sx={{ color: "white" }}
              color="inherit"
            ></CircularProgress>
            ...
            <div>
              {!watch(`video${singleSec.num}`)?.length ? (
                <label
                  className="text-red-500  underline p-4 inline-block  cursor-pointer "
                  htmlFor={"photosFile" + singleSec.num}
                >
                  {watch(`img${singleSec.num}`)?.length
                    ? "Change Images"
                    : "Add Images"}
                </label>
              ) : (
              ...
              {watch(`img${singleSec.num}`)?.length ? (
                <button
                  type="button"
                  className="text-blue-500 ml-3 underline cursor-pointer"
              ...
              {!watch(`img${singleSec.num}`)?.length ? (
                <label
                  htmlFor={"file" + singleSec.num}
                  className="text-green-500 underline cursor-pointer p-4 inline-block "
                >
                  {watch(`video${singleSec.num}`)?.length
                    ? "Change video"
                    : "Add video"}
      ...
      <div className="w-full flex justify-center items-center mt-5">
        <Tooltip title="Delete upper section">
          <span className="bg-gray-900 rounded-full">
            {singleSec.num != 1 && (
              <IconButton
                onClick={() => handleDelete(singleSec.num)}
                sx={{ color: "red" }}
              >
                <CloseIcon></CloseIcon>
              </IconButton>
        ...
        <div className=" h-screen bg-black py-5 pl-4  flex flex-col">
          <div className="grow overflow-y-scroll dashboard-scrollBar p-2 ">
            <div>
              <Grid container spacing={2}>
                {watch(`img${singleSec.num}`)?.map((single, i) => (
                  <Grid key={i} item xs={12} md={3}>
                    ...
                    ></Image>
                    <input
                      className="w-full p-3 rounded-lg  bg-gray-900 placeholder:text-slate-400 text-white"
                      type="text"
          ...
          <div className="flex-none ">
            <button
              ...
              className="mt-2 rounded-md bg-yellow-500 px-4 p-2"
            >
              Done
            </button>
          </div>
