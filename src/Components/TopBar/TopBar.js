...
const TopBar = () => {
  ...
  const { data, error } = useSWR(
    "https://jhon-portfolio-server.vercel.app/chooseMenu",
    fetcher
  );
  useEffect(() => {
    dispatch(addCollection(data));
  }, [data, dispatch, user, error, route.pathname]);
  ...
  const Element = (
    <motion.div variants={stagger} className="px-2 py-5 ">
      <motion.div variants={fadeEnter}>
        <Link href={"/"}>
          <span className="w-full border  flex justify-center mb-3 py-2 border-gray-700 rounded-sm hover:border-yellow-300 text-white">
            Home
          </span>
        </Link>
      </motion.div>
...
https://i.ibb.co/MV6YcKV/huisstijl-JBP-750.png
              ...
              <Image
                ...
                height={200}
                layout="raw"
                src="https://i.ibb.co/kMjgnHb/huisstijl-JBP-500.png"
                className="mt-1"
                alt="logo"
              ></Image>
            </motion.div>
            ...
            >
              ...
              >
                <Box
                  ...
                  sx={{ background: "black", width: "50vw" }}
                >
                  {Element}
                </Box>
              </Drawer>
            ...
            >
              <motion.div className=" flex" variants={stagger}>
                <motion.div className="px-5" variants={fadeInUp}>
                  <CustomLink href={"/"}>
                    <Button sx={{ py: 0, color: "white", display: "block" }}>
                      Home
                    </Button>
                  </CustomLink>
                </motion.div>
                <motion.div
                  className="menu-collection-wrap-main pr-5"
                  variants={fadeInUp}
                >
                  <Button sx={{ py: 0, color: "white", display: "block" }}>
                    Collection
