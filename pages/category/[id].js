        ...
        <div>
          <Container>
            <div className=" my-20 py-5">
              <h2 className="text-4xl font-thin mb-5 mt-5 text-heading">
                {data?.title}
              </h2>
              <p className="text-contentText">{data?.description}</p>
            </div>
...
export async function getServerSideProps({ params }) {
  const res = await fetch(
    `https://jhon-portfolio-server.vercel.app/singleCategory?id=${params.id}`
  );
