...
const UpdateBlog = () => {
  ...
  const { data: res, error } = useSWR(
    `https://jhon-portfolio-server.vercel.app/blog?id=${id}`,
    fetcher
  );
  ...
  const onSubmit = (mainData) => {
    ...
    axios
      .put(
        `https://jhon-portfolio-server.vercel.app/blog?id=${data._id}`,
        { mainData: create, user: user?.email },
        {
          headers: {
  ...
  return (
    <div>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div>
          <div
            ...
            style={{ height: "50vh", backgroundImage: `url(${data.img})` }}
          >
            <div className="absolute inset-0 flex justify-center items-center bg-black/[.5]">
              {imgLoading ? (
                ...
                <label
                  htmlFor={"img"}
                  className="border-b pointer-events-auto border-green-400 text-green-400 cursor-pointer mr-4 "
                >
                  Change thumbnails
                </label>
              )}
          ...
          <input
            ...
            className=" text bg-transparent   text-xl md:text-3xl text-center text-heading font-family-mono font-thin border-white block w-full px-4  py-2"
          />
          <input
            type="text"
            {...register("address", { required: true })}
            defaultValue={data.address}
            className=" text-xl text-center text-subTitles font-family-mono bg-transparent  font-family-mono  border-white block w-full px-4  py-2"
          />
        </div>
        <div className="block md:flex justify-between items-center pb-5 md:pb-0">
          <div className="pb-5 flex mt-5">
            <div className="mr-4">
              <Avatar
                src="https://i.ibb.co/5LwPHgJ/20190320-WEST-AFRICA-FROM-GUNJUR-TO-BASSE-2964-removebg-preview.png"
                alt="blog "
                sx={{
                  width: 40,
                  height: 40,
            ...
            <div className="border-l border-gray-800 pl-3">
              ...
              <h3 className="text-sm font-light text-contentText">
        ...
        <div>
          <textarea
            ...
            className="text-white bg-transparent   border-white block w-full px-4  py-2"
            cols="5"
            rows="5"
          />
        </div>
        {data?.sections.map((single, i) => (
          <BlogSectionUpdate
            key={i}
            setData={setData}
            index={i}
        ...
        <div>
          ...
          <div className="my-5   ">
            {watch("tags")?.map((tag, i) => (
              <span
                className="inline-block text-white mb-2 p-2 border rounded-full mr-2"
                key={i}
              >
                #{tag}
                <IconButton
                ...
                >
                  <ClearIcon
                    sx={{ fontSize: "20px", color: "red", p: 0 }}
                  ></ClearIcon>
                </IconButton>
              </span>
            ))}
      ...
      {upLoading ? (
        ...
        <label
          htmlFor="submit"
          className="text-yellow-500 inline-block px-4 py-2 mt-5 border border-yellow-500"
          type="submit"
        >
          Update blog
        </label>
