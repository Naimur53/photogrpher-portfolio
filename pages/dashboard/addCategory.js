...
const AddCategory = ({ uniqCategory }) => {
  ...
  const onSubmit = (data) => {
    ...
    axios
      .post(
        "https://jhon-portfolio-server.vercel.app/category",
        { mainData: data, user: user?.email },
        {
          headers: {
  ...
  useEffect(() => {}, [handleChangeUrl, watch, setValue]);
  const photosUrls = () => {
    let bgUrl;
  ...
  return (
    <div>
      <h2 className="text-2xl text-white capitalize text-center my-5">
        Add a new category of your image
      </h2>
      <form
      ...
      >
        <Grid container spacing={4}>
          ...
          <Grid xs={12} md={6} item>
            ...
            >
              <input
                ...
                accept="image/*"
                id="thumbnailFile"
                className=" z-20 w-full h-full opacity-0"
              />
              <div className="absolute inset-0 flex justify-center items-center">
                {thumbnailLoading ? (
                  ...
                  <label
                    ...
                    className="bg-black/[.7] text-white p-2 rounded-md cursor-pointer"
                  >
                    Choose Thumbnail
                  </label>
                )}
          ...
          <Grid xs={12} md={6} item>
            <Box
              className={`h-40 relative dashboard-scrollBar  bg-gray-900 ${
                imgLoading ? "overflow-hidden" : "overflow-x-auto"
              }`}
            >
              <div className="absolute pointer-events-none inset-0 flex justify-center items-center ">
                <input
                  ...
                  id="url"
                  accept="image/*"
                  className="z-50 pointer-events-auto h-full w-full opacity-0"
                  multiple={true}
                />
                <div className="absolute inset-0 flex justify-center items-center">
                  {imgLoading ? (
                    <div className="z-10 w-full h-full flex justify-center items-center backdrop-blur-sm">
                      {" "}
                      <CircularProgress
                        sx={{ color: "white" }}
                        color="inherit"
                    ...
                    <label
                      htmlFor="url"
                      className="z-10 bg-black/[.7] text-white p-2  rounded-md pointer-events-auto cursor-pointer"
                    >
                      Choose Images
                    </label>
                  )}
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={3}>
                      <div className="p-1 h-full">
                        <label
                          htmlFor="url"
                          className="flex justify-center items-center h-full w-full bg-black/[.7] p-2 text-white  rounded-md text-center pointer-events-none"
                        >
        ...
        {!imgLoading && !thumbnailLoading ? (
          <div className="mt-5">
            <input
              className="bg-contentText cursor-pointer px-4 rounded-md text-black p-2"
              type="submit"
            />
          </div>
        ) : (
          <div className="mt-5">
            <span className="bg-red-400 rounded-md text-black p-2">
              Loading Images
            </span>
          </div>
        )}
...
export async function getServerSideProps() {
  ...
  const res = await fetch(
    `https://jhon-portfolio-server.vercel.app/uniqCategory`
  );
