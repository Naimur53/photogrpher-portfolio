...
const UpdateCategory = () => {
  ...
  const { data: res, error } = useSWR(
    `https://jhon-portfolio-server.vercel.app/singleCategory?id=${id}`,
    fetcher
  );
  useEffect(() => {
    setData(res);
  ...
  const handleDelete = () => {
    const remain = data?.photos.filter(
      (single) => !selected.find((select) => select === single)
    );
  ...
  const handleThumbnailFile = (e) => {
    ...
    if (file.length) {
      ...
      axios({
        ...
        .catch((e) => {
          toast.error(
            "Something bad happened to upload image massage is " + e.message,
            {
              position: "bottom-right",
              autoClose: 5000,
              hideProgressBar: false,
  ...
  const onSubmit = (editData) => {
    ...
    axios
      .put(
        `https://jhon-portfolio-server.vercel.app/category?id=${data._id}`,
        { mainData: upData, user: user.email },
        {
          headers: {
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        >
          <div className="absolute inset-0 flex justify-center items-center flex-col">
            <h2 className="text-4xl font-family-Helvetica text-center ">
              {data.subCategory ? (
                <span>
            ...
            {thumbnailLoading ? (
              ...
              <label
                ...
                className="bg-black/[.7] cursor-pointer inline-block p-2 rounded-full"
              >
                <CameraEnhanceIcon sx={{ color: "white" }}></CameraEnhanceIcon>
              </label>
            )}
          </div>
          <div className="flex justify-end absolute top-0 right-0 pb-4">
            <div>
              {uploadLoading ? (
                <CircularProgress sx={{ color: "white" }}></CircularProgress>
              ) : imgLoading || thumbnailLoading ? (
                <span className="p-2 bg-red-900 m-4 text-white  ">
                  Image Loading
                </span>
              ) : (
                <button type="submit" className="p-2 bg-red-900 m-4 text-white">
                  Update
                </button>
              )}
            </div>
        ...
        <div className="py-5 text-center">
          <input
            ...
            className="text-white bg-transparent   border-white inline-block w-full text-center py-2 mb-4 text-xl"
          />
          <textarea
            type="text"
            defaultValue={data.description}
            {...register("description", { required: true })}
            className="text-white bg-transparent   border-white inline-block w-full text-center py-2"
          />
        </div>
        <div className="mb-4 mr-4">
          ...
          {imgLoading ? (
            ...
            <label
              htmlFor="url"
              className="text-center pointer-events-auto text-green-500 border-b border-green-500  mt-2 cursor-pointer "
            >
              Add more images
            </label>
          )}
          ...
          {selected.length ? (
            <span
              ...
              className="text-center  pointer-events-auto cursor-pointer text-red-500 ml-4 border-b border-red-500 mr-4  mt-2  "
            >
              Delete Selected image
            </span>
          ) : (
            <span className="text-center grayscale pointer-events-auto cursor-pointer text-red-500 border-b border-red-500 ml-4  mt-2  ">
              Delete Selected image
            </span>
          )}
          <button
            ...
            className="text-center  pointer-events-auto cursor-pointer text-pink-500 ml-4 border-b border-pink-500 mr-4  mt-2"
          >
            Change Layout
          </button>
        </div>
            ...
            {[1, 2, 3, 4].map((element, i) => {
              ...
              return (
                <Grid key={i} item md={3}>
                  {data.photos?.slice(len * i, len * element).map((res, i) => (
                    <CategorySingleImg
                      selected={selected}
