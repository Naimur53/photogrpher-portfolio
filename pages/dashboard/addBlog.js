...
const AddBlog = () => {
  ...
  const onSubmit = (data) => {
    ...
    axios
      .post(
        "https://jhon-portfolio-server.vercel.app/blog",
        { mainData, user: user?.email },
        {
          headers: {
  ...
  return (
    <>
      <form className=" " onSubmit={handleSubmit(onSubmit)}>
          ...
          <Grid container spacing={4}>
            ...
            <Grid item xs={12} md={6}>
              <textarea
                ...
                className="w-full dashboard-scrollBar p-3 rounded-lg  bg-gray-900 placeholder:text-slate-400 text-white"
            ...
            <Grid item xs={12} md={6}>
              ...
              >
                <input
                  ...
                  accept="image/*"
                  id="mainImg"
                  className="w-full h-full z-20 opacity-0"
                />
        ...
        <div>
          <h2 className=" my-5 text-center text-2xl text-white">
            Add sections
          </h2>
          <Grid container spacing={4}>
            {numSection.map((singleSec) => (
              <CreateBlogSection
          ...
          {numSection[numSection.length - 1].complete && (
            <button
              className="bg-yellow-500 mt-5 text-gray-900 p-4 rounded"
              onClick={() => {
                setNumSection([
                  ...numSection,
                  { complete: false, num: numSection.length + 1 },
                ]);
              }}
            >
              Add another section
      ...
      {imgLoading || photosLoading || videoLoading || postLoading ? (
        <button className="bg-red-400 text-black rounded-md px-5 text-xl py-2 my-5 inline-block ">
          wait until loading
        </button>
      ) : (
        <label
          htmlFor="submit"
          className="bg-contentText text-black rounded-md px-5 text-xl py-2 my-5 inline-block "
        >
          submit
        </label>
      )}
